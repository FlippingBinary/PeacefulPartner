...
export type CurrentSource = {
  getFrame: (maxSize: any) => Promise<any>,
  start: () => Promise<any>,
  stop: () => void,
  stopped: boolean,
  isDummySource: boolean
};

/*Prevents globalThis being reported as an error by eslint*/
/*global globalThis*/
export class Source {
  // define propeties of the class
   ...
   */
  async useVideoElement(videoEl: HTMLVideoElement): Promise<any> {
   ...
   */
  async getAvailableCameraList(): Promise<MediaDeviceInfo[]> {
   ...
   * @returns a Promise resolved as soon as the operation is completed, or rejected in case of error
   */
  async useCamera({ deviceId, toVideoElement, constraints, flip }: UseCameraOptions = {}): Promise<any> {
